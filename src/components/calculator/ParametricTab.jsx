...
export default function ParametricTab({
  ...
  const handleUpdateParametric = (id, field, value) => {
    ...
    // When an expression changes, re-compile it using mathjs parser
    try {
        if (field === "xExpression") {
            updates.compiledX = MathParser.parseFunction(value)
        } else if (field === "yExpression") {
            updates.compiledY = MathParser.parseFunction(value)
        }
    } catch (error) {
        console.error("Error parsing updated parametric function:", error)
  ...
  const presetCurves = [
    { name: "Circle", x: "cos(t)", y: "sin(t)", tMin: 0, tMax: 6.28, description: "Perfect unit circle", color: "from-blue-500 to-cyan-500" },
    { name: "Ellipse", x: "3*cos(t)", y: "2*sin(t)", tMin: 0, tMax: 6.28, description: "Ellipse with a=3, b=2", color: "from-green-500 to-emerald-500" },
    { name: "Spiral", x: "t*cos(t)", y: "t*sin(t)", tMin: 0, tMax: 12.56, description: "Archimedean spiral", color: "from-purple-500 to-pink-500" },
    { name: "Lissajous", x: "sin(3*t)", y: "sin(2*t)", tMin: 0, tMax: 6.28, description: "Lissajous curve (3:2)", color: "from-orange-500 to-red-500" },
    { name: "Cycloid", x: "t - sin(t)", y: "1 - cos(t)", tMin: 0, tMax: 12.56, description: "Path of a rolling point", color: "from-indigo-500 to-purple-500" },
    { name: "Rose", x: "cos(4*t)*cos(t)", y: "cos(4*t)*sin(t)", tMin: 0, tMax: 6.28, description: "4-petaled rose", color: "from-pink-500 to-rose-500" },
  ]
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-6">
          <div className="backdrop-blur-xl bg-slate-800/40 border border-slate-600/30 rounded-2xl p-5 shadow-lg">
            <Label className="text-slate-300 text-sm mb-3 block font-medium">Quick Symbols (for x(t) input)</Label>
            <div className="grid grid-cols-4 gap-2">
              {mathSymbols.map((sym) => (
                <Button
                  key={sym.symbol}
                  onClick={() => addSymbolToInput(sym.value, setXExpression)}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="backdrop-blur-xl bg-slate-800/40 border border-slate-600/30 rounded-2xl p-6 shadow-lg">
            ...
            <div className="text-slate-300 text-sm space-y-4">
              <p>Parametric equations define curves using a parameter, usually 't', where both x and y coordinates are functions of t.</p>
              ...
              <p>This allows you to create complex curves that can't be expressed as a simple y = f(x), such as circles, spirals, and loops.</p>
            </div>
          </div>
        </div>
      </div>
